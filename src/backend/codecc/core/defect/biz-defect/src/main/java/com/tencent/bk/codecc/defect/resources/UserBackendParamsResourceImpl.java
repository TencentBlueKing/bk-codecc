...
public class UserBackendParamsResourceImpl implements UserBackendParamsResource {
    ...
    public Result<Boolean> pubMsgToCommitDefect(String mode, String jsonBody) {
        ...
        String exchange =
                String.format("%s%s.%s", ConstantsKt.PREFIX_EXCHANGE_DEFECT_COMMIT, toolPattern.toLowerCase(), prefix);
        String routingKey =
                String.format("%s%s.%s", ConstantsKt.PREFIX_ROUTE_DEFECT_COMMIT, toolPattern.toLowerCase(), prefix);
