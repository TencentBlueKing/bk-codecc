...
public abstract class AbstractDefectTracingComponent<T> {
    ...
    public AsyncRabbitTemplate.RabbitConverterFuture<Boolean> executeCluster(
        ...
        try {
            ...
            if (ComConstants.BsTaskCreateFrom.GONGFENG_SCAN.value().equalsIgnoreCase(taskDetailVO.getCreateFrom())) {
                ...
                asyncMsgFuture = asyncRabbitTamplte.convertSendAndReceive(EXCHANGE_CLUSTER_ALLOCATION_OPENSOURCE,
                        ROUTE_CLUSTER_ALLOCATION_OPENSOURCE, defectClusterDTO);
            } else {
                AsyncRabbitTemplate asyncRabbitTamplte = SpringContextUtil.Companion.getBean(AsyncRabbitTemplate.class,
                        "clusterAsyncRabbitTamplte");
                asyncMsgFuture = asyncRabbitTamplte.convertSendAndReceive(EXCHANGE_CLUSTER_ALLOCATION,
                        ROUTE_CLUSTER_ALLOCATION, defectClusterDTO);
            }
    ...
    public Map<String, String> getFIleMd5Map(String streamName, String toolName, String buildId) {
        ...
        return fileMD5TotalModel.getFileList().stream()
                .collect(Collectors.toMap(FileMD5SingleModel::getFilePath, FileMD5SingleModel::getMd5));
    }
    ...
    protected void checkOutputFileExists(String outputFile) throws InterruptedException {
        ...
        while (!file.exists() && i < 200) {
            scmJsonComponent.indexNoThrow(outputFileName, ScmJsonComponent.AGGREGATE);
            Thread.sleep(3000L);
            i++;
        }
    }
