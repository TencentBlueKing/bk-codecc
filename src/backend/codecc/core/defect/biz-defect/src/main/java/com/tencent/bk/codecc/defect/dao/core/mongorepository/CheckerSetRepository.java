...
public interface CheckerSetRepository extends MongoRepository<CheckerSetEntity, String> {
     ...
     * @return entity
     */
    CheckerSetEntity findFirstByCheckerSetId(String checkerSetId);
    ...
    @Query(fields = "{'checker_set_id':1}")
    List<CheckerSetEntity> findByCheckerSetIdIsNotNull(Pageable pageable);

    @Query(fields = "{'checker_set_id':1,'checker_props':1,'version':1}")
    List<CheckerSetEntity> findByVersionIn(Collection<Integer> versions, Pageable pageable);
}
