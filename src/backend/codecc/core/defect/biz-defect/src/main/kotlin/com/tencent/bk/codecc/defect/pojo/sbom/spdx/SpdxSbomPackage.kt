...
class SpdxSbomPackage(
    ...
    fun getSbomPackage(taskId: Long, toolName: String, status: Int): SCASbomPackageEntity {
        ...
        val licensesDeclared = licenseDeclared?.split(PACKAGE_LICENSE_SEPARATOR)?.filter(StringUtils::isNotBlank)
            ?.map { it.trim() } ?: emptyList()
        val licensesConcluded = licenseConcluded?.split(PACKAGE_LICENSE_SEPARATOR)?.filter(StringUtils::isNotBlank)
            ?.map { it.trim() } ?: emptyList()
        sbomPackage.licensesDeclared = licensesDeclared
        sbomPackage.licensesConcluded = licensesConcluded
        sbomPackage.licenses = licensesDeclared + licensesConcluded
