...
@Component
class ClusterCommitIdCompareProcess: AbstractClusterCompareProcess<LintDefectV2Entity>() {
    /**
     * 聚类方法
     */
    override fun clusterMethod(
        ...
        val defectMap = inputDefectList.groupBy {
            it.relPath to it.revision to it.checker
        }
        defectMap.forEach { (_, u) ->
            val outputFileMap = u.groupBy { it.lineNum }
     ...
     * 获取特征值映射方法，每个类型方法不一致
     */
    override fun getPinpointHashMap(
        aggregateDefectInputList: List<LintDefectV2Entity>
    ): Map<String?, List<LintDefectV2Entity>> {
        return mapOf()
    }
