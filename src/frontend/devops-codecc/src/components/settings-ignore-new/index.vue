<template>
  <bk-dialog
    ...
    <bk-tab type="unborder-card" :label-height="42" class="create-tab" @tab-change="changeTab">
      <bk-tab-panel
        ...
        <div class="input-paths">
          <bk-form :label-width="0" :model="formData">
            <bk-form-item
              ...
              :key="index">
              <bk-input :placeholder="$t('请输入屏蔽路径')" class="input-style" v-model.trim="formData.paths[index].customPath"></bk-input>
      ...
      save() {
        if (this.tabSelect === 'input') {
          ...
          for (let i = 0; i < this.formData.paths.length; i++) {
            ...
            } else if (this.checkPath(this.formData.paths[i].customPath)) { // 添加非法路径
              this.$bkMessage({ theme: 'error', message: this.$t('请勿屏蔽全部代码，否则会造成代码检查工具失败，从而遗漏问题风险') })
              return
