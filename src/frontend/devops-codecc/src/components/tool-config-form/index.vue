<template>
  <div v-if="scenes === 'manage-edit' && ((taskDetail.atomCode && taskDetail.createFrom === 'bs_pipeline')
    || taskDetail.createFrom === 'gongfeng_scan' || taskDetail.createFrom === 'api_trigger')" :model="formData" ref="codeForm">
    <div class="disf">
      ...
      <ul v-else>
        <li class="fs14 pb5" v-for="item in codeMessage.codeInfo" :key="item.aliasName">
    ...
    <div class="disf" v-if="isGitRepo || branchList.length">
      ...
      <ul>
        <li class="fs14 pb5" v-for="item in branchList" :key="item">{{item}}</li>
      </ul>
    </div>
    ...
    computed: {
      ...
      shownTool() {
        ...
        this.tools.forEach((toolName) => {
          if (['COVERITY', 'KLOCWORK', 'PINPOINT', 'CODEQL', 'CLANG', 'SPOTBUGS'].includes(toolName)
            && this.hasScript) {
            shownTool = true
          }
        })
      ...
      handleSubmit(event) {
        this.$refs.codeForm.validate().then((validator) => {
          ...
          if (this.scenes === 'manage-edit') {
            ...
            const obj = {}
            tools = tools.reduce((item, next) => { // 去重
