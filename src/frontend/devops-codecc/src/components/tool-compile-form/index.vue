  ...
  methods: {
    getCompileToolList(val) {
      if (val) {
        ...
        this.$store
          .dispatch('task/getCompileTool', val)
          .then((res) => {
      ...
      if (newValue !== '') {
        this.compileToolList.forEach((item, index) => {
          if (item.name === newValue) {
            if (
              vm.compileToolVersionList.filter(item => item.name === newValue)
                .length === 0
            ) {
              if (
                ...
                vm.compileToolVersionList[
                  vm.compile.compileTool.findIndex(item => item === newValue)
                ] = item.versions;
                vm.compile.compileToolVersion[
                  vm.compile.compileTool.findIndex(item => item === newValue)
                ] = '';
                vm.compileToolVersionList = Array.from(new Set(vm.compileToolVersionList));
              }
