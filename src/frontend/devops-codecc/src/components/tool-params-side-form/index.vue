  ...
  <div class="params-side" :class="{ mini: !isParamsSideFormShow }">
    <div v-show="isParamsSideFormShow">
      <div class="main-top">
        ...
        <span class="fs12" v-if="hasSave">{{
          $t('已接入x个工具', { num: toolsNum })
        }}</span>
        <span class="fs12" v-else>{{
          $t('已选x个工具', { num: toolsNum })
        }}</span>
        <span
          class="fr bk-icon icon-angle-right toggle-button"
          @click="isParamsSideFormShow = false"
        ></span>
      </div>
      <bk-form class="side-card" v-model="toolConfigParams">
        <toolparams>
          ...
          >
            ...
            <bk-form-item
              v-if="
                toolParam.toolName !== 'CCN' && toolParam.toolName !== 'DUPC'
              "
              :label="$t('single.规则集')"
    ...
    <div v-show="!isParamsSideFormShow">
      <div class="main-top">
        {{ $t('工具配置') }}
        <span
          class="fl bk-icon icon-angle-left toggle-button mr10"
          @click="isParamsSideFormShow = true"
        ></span>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getParamsValue() {
      ...
      const tools = this.toolConfigParams.map((item) => {
        ...
        paramsList.forEach((param) => {
          ...
          paramObj[key] = (this.paramsValue[toolName] && this.paramsValue[toolName][key])
            || param.varDefault;
        });
    ...
    submitData() {
      ...
      this.$store
        .dispatch('tool/updateParamsAndCheckerSets', postData)
        .then((res) => {
          if (res === true) {
