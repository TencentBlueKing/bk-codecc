...
export default {
  ...
  actions: {
    lintDetail({ commit, state, dispatch }, params, config = {}) {
      ...
      return http.post('/defect/api/user/warn/issue/detail', data, { params: query, cancelPrevious: false }).then((res) => {
    ...
    lintList({ commit, state, dispatch }, params, config = {}) {
      ...
      return http.post('/defect/api/user/warn/issue/list', data, { params: query }).then((res) => {
        const list = res.data || {}
    ...
    publish({ commit, state, dispatch }, index) {
      ...
      return http.post('/tool/publish', data).then((res) => {
    ...
    report({ commit, state, dispatch }, data) {
      ...
      return http.get(`/defect/api/user/report/toolName/${data.toolId}`, { params: query }).then((res) => {
    ...
    fileContent({ commit, state, dispatch }, params) {
      return http.post('/defect/api/user/warn/fileContentSegment', params).then((res) => {
        const detail = res.data || {}
        return detail
      })
        .catch((e) => {
    ...
    getOperatreRecords({ commit, state, dispatch, rootState }, data) {
      ...
      return http.post(`/defect/api/user/operation/taskId/${data.taskId}`, data.funcId).then((res) => {
    ...
    newVersion({ commit, state, dispatch }, params) {
      return http.post(`/defect/api/user/checker/tools/${params.toolName}/checkerSets/${params.checkerSetId}/versions/difference`, params).then((res) => {
        const data = res.data || {}
        return data
      })
        .catch((e) => {
    ...
    getWarnContent({ commit, state, dispatch }, data) {
      return http.get(`/defect/api/user/checker/detail/toolName/${data.toolName}?checkerKey=${data.checkerKey}`, data).then((res) => {
    ...
    getTransferAuthorList({ commit, rootState }) {
      return http.get('/defect/api/user/transferAuthor/list').then((res) => {
        const data = res.data || {}
        return data
      })
        .catch((e) => {
    ...
    getBuildList({ commit, rootState }, data) {
      // TODO: 多分支接口暂时回滚
      return http.get(`/defect/api/user/warn/tasks/${data.taskId}/buildInfosWithBranches`).then((res) => {
    ...
    commentDefect({ commit, state, dispatch }, params) {
      ...
      return http.post(`/defect/api/user/warn/codeComment/toolName/${query.toolName}`, data, { params: query }).then(res => res || {})
        .catch((e) => {
          console.error(e)
        })
    },
    deleteComment({ commit, state, dispatch }, params) {
      const { commentId, singleCommentId, toolName, defectEntityId, commentStr } = params
      return http.delete(`/defect/api/user/warn/codeComment/commentId/${commentId}/singleCommentId/${singleCommentId}/toolName/${toolName}`, { params: { entityId: defectEntityId, comment: commentStr } }).then(res => res || {})
        .catch((e) => {
          console.error(e)
        })
    },
    updateComment({ commit, state, dispatch }, params) {
      ...
      const data = { singleCommentId, userName, comment }
      return http.put(`/defect/api/user/warn/codeComment/commentId/${commentId}/toolName/${toolName}`, data).then(res => res || {})
        .catch((e) => {
          console.error(e)
        })
    },
