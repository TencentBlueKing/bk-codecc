  ...
  <div :class="['task-card', { 'disabled-bg': isDisableTask }]" :style="{ height: cardHeight }">
        ...
        <hgroup>
          ...
          <bk-popover theme="light" class="codelib-popover" :delay="[600, 10]" placement="bottom"
            ...
            <section ref="codelibContent">
              <section>
                <p class="task-desc" v-for="item in task.codeLibraryInfo.codeInfo.slice(0,5)" :key="item.aliasName">
            ...
            <div slot="content">
              <section>
                <p class="codelib-item" v-for="item in task.codeLibraryInfo.codeInfo" :key="item.aliasName">
                  {{ `${item.aliasName}@${item.branch}` }}
                </p>
              </section>
            </div>
    ...
    <section class="analysis-contnet">
      ...
      <div class="analysis-statistics" v-else>
        <div class="col-item">
          <p :class="['col-value', { 'disabled': task.status === 3 }]">{{ 'totalDefectCount' in task ? task.totalDefectCount : '--' }}</p>
        ...
        <div class="col-item">
          <p :class="['col-value', { 'disabled': task.status === 3 }]">{{ 'totalSecurityDefectCount' in task ? task.totalSecurityDefectCount : '--' }}</p>
        ...
        <div class="col-item">
          <p :class="['col-value', { 'disabled': task.status === 3 }]">{{ 'totalStyleDefectCount' in task ? task.totalStyleDefectCount : '--' }}</p>
    ...
    <section class="handle-content">
      <template v-if="!isDisableTask">
        <i class="bk-icon icon-play-circle-shape fs20 disabled" v-if="task.hasNoPermission"></i>
        <i class="bk-icon icon-play-circle-shape fs20" v-else-if="[0, 1].includes(stepStatus) || isWaitingTask" @click="handleTask(task, 'execute')"></i>
    ...
    computed: {
      ...
      taskTools() {
        Object.values(this.task.toolConfigInfoList).forEach((value) => {
          if (value.curStep < 5 && value.curStep > 0
            && value.stepStatus !== 1) {
            value.stepStatus = 3
          }
        })
      ...
      isWaitingTask() {
        return this.task.displayStepStatus === undefined
          || (this.task.displayStepStatus === 0 && !this.task.displayStep)
      },
      // 自定义状态枚举： 0-成功 1-失败 2-等待中 3-分析中
      stepStatus() {
        let stepStatus = 0
        if (this.task.displayStep && this.task.displayStep < 5 && this.task.displayStep > 0
