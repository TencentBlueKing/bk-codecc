    ...
    <bk-collapse v-model="activeName">
      <bk-collapse-item content-hidden-type="hidden" name="message">
        ...
        <div slot="content">
          <div v-if="!isEditable">
            ...
            <label class="pipeline-content">{{ (recieveList.find(item => item.id === formData.rtxReceiverType) || {}).name }}</label>
            ...
            <label v-if="formData.rtxReceiverType === '4'" class="f12">{{$t('暂不包含重复率工具重复文件的相关作者')}}</label>
          </div>
      ...
      <bk-collapse-item content-hidden-type="hidden" name="robot">
        ...
        <div slot="content">
          <div v-if="!isEditable">
            ...
            <label class="pipeline-content">{{ $t('工具：') }}{{ botToolListPicked.length ? botToolListPicked.join() : $t('暂无合适工具') }}</label>
          </div>
          <bk-form v-else ref="robot" :model="formData">
            ...
            <bk-form-item :label="$t('通知内容')">
              <span class="label-tips">
                ...
                {{$t('任务分析完成后，如果待修复问题数量未清零，就立即通过企业微信群机器人发送分析结果并@问题处理人')}}
              </span>
              <bk-form-item class="form-item-moved" :label="$t('问题数量')" :label-width="$t('100')">
                <bk-radio-group class="pb10" v-model="formData.botRemindRange" name="botRemindRange">
                  <bk-radio-button :value="2">{{$t('新+存量问题')}}</bk-radio-button>
                  <bk-radio-button :value="1">{{$t('新问题')}}</bk-radio-button>
                </bk-radio-group>
                <bk-radio-group v-model="formData.botRemindSeverity" name="botRemindSeverity">
              ...
              <bk-form-item class="form-item-moved" :label="$t('工具选择')" :label-width="$t('100')">
                ...
                <!-- <tool-selector v-if="botToolList.length" :list="botToolList" :picked="formData.botRemaindTools" @select="selectedBotTools"></tool-selector> -->
      ...
      <bk-collapse-item content-hidden-type="hidden" name="mail">
        ...
        <div slot="content">
          <div v-if="!isEditable">
            <label class="pipeline-item bk-label">{{ $t('接收人') }}</label>
            <label class="pipeline-content">{{ (recieveList.find(item => item.id === formData.emailReceiverType) || {}).name }}</label>
            ...
            <label v-if="formData.emailReceiverType === '4'" class="f12">{{$t('暂不包含重复率工具重复文件的相关作者')}}</label>
            <div style="height: 15px;"></div>
          ...
          <bk-form v-else ref="mail" :model="formData">
            ...
            <bk-form-item :label="$t('即时报告')">
              ...
              <div class="switcher">
                <bk-switcher :value="!!instantReportStatus" @click.native="instantReport" size="min" theme="primary"></bk-switcher>
  ...
  export default {
    ...
    computed: {
      ...
      toolListMap() {
        ...
        const list = enableToolList.concat({ toolName: 'ALL', toolDisplayName: '所有工具' })
