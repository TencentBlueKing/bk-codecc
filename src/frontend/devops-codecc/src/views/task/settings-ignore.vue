<template>
  <div>
    <bk-tab class="settings-ignore-tab" :label-height="42" :active.sync="active" type="unborder-card" @tab-change="changeTab">
      ...
      <div class="path-list" v-if="customExist">
        <span v-if="projectId === 'CUSTOMPROJ_PCG_RD'" class="link-text">{{$t('由PCG EP度量平台创建的任务，暂不支持在页面自定义屏蔽路径')}}</span>
        <span v-else-if="!isEditable" class="link-text">{{$t('修改路径屏蔽，请前往流水线')}} <a @click="hanldeToPipeline" href="javascript:;">{{$t('立即前往>>')}}</a></span>
        <div class="settings-ignore-header" v-if="customExist && isEditable && projectId !== 'CUSTOMPROJ_PCG_RD'">
      ...
      <div class="no-path" v-if="customNone">
        <empty size="small" title="" :desc="$t('添加后的代码路径将被屏蔽，不再有问题等产生')">
          <template v-slot:action>
            <span v-if="projectId === 'CUSTOMPROJ_PCG_RD'" class="fs12">{{$t('由PCG EP度量平台创建的任务，暂不支持在页面自定义屏蔽路径')}}</span>
            <span v-else-if="!isEditable" class="fs12">{{$t('修改路径屏蔽，请前往流水线')}} <a @click="hanldeToPipeline" href="javascript:;">{{$t('立即前往>>')}}</a></span>
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(name) {
        ...
        this.$store.dispatch('task/deleteIgnore', { params }).then((res) => {
          if (res === true) {
