<template>
  <div class="main-content-inner main-content-form" :class="editDisabled ? 'from-pipeline' : ''">
    <div v-if="editDisabled" class="f14" :model="issueData">
      <div v-if="taskDetail.createFrom === 'api_trigger'" class="fs12 to-pipeline">{{$t('问题提单配置由CodeCC开源扫描集群自动生成')}}</div>
      <div v-else class="to-pipeline">
        <span>{{$t('修改问题提单配置，请前往流水线')}} <a @click="hanldeToPipeline" href="javascript:;">{{$t('立即前往>>')}}</a></span>
      </div>
    ...
    <div v-else>
      <bk-form :label-width="130" :model="issueData" ref="issueInfo">
        <bk-form-item :label="$t('提单类型')" property="system" :required="true" :rules="rules.required">
          <bk-radio-group v-model="issueData.system">
            <bk-radio v-for="item in issueData.issueSystemInfoVOList || []" :key="item.system" :value="item.system">{{item.detail}}</bk-radio>
          </bk-radio-group>
        </bk-form-item>
        <bk-form-item :label="$t('Tapd库')" property="subSystem" :required="true" :rules="rules.required">
          <bk-popover v-if="issueData.subSystemId" placement="right" theme="light">
            <bk-link theme="primary">{{issueData.subSystemCn}}</bk-link>
            <div slot="content">
              <bk-link theme="primary" @click="jumpTo('tapd')">{{$t('去Tapd')}}</bk-link>
              <span class="split"></span>
              <bk-link theme="primary" @click="jumpTo('change')">{{$t('切换授权')}}</bk-link>
            </div>
          </bk-popover>
        ...
        <bk-form-item :label="$t('提单上限')" :desc="$t('若Tapd库中待修复问题数超过提单上限，将不再提单。最大上限为10000条。')">
          <bk-input v-model="issueData.maxIssue" type="number" :min="0" :max="10000" :placeholder="$t('默认为1000条')"></bk-input>
        </bk-form-item>
        ...
        <bk-form-item>
          <bk-button theme="primary" @click="save">{{$t('保存')}}</bk-button>
        </bk-form-item>
      </bk-form>
    </div>
    ...
    computed: {
      ...
      toolList() {
        return this.taskDetail.enableToolList.filter(item => (item.toolName !== 'CLOC'
          && item.toolName !== 'STAT' && item.toolName !== 'SCC' && item.toolName !== 'DUPC'))
      },
      taskId() {
...
${this.taskDetail.pipelineId}/edit#${this.taskDetail.atomCode}`, '_blank')
      ...
      async save() {
        ...
        if (res.code === '0') {
          ...
          this.$bkMessage({ theme: 'error', message: res.message || this.$t('保存失败') })
        }
      },
      jumpTo(to) {
      ...
      handleSeverity(severity = []) {
        const list = this.severityList.filter(item => severity.includes(item.id)).map(item => item.name)
        return list.join(', ')
      },
      handleTools(tools = []) {
        const list = this.toolList.filter(item => tools.includes(item.toolName)).map(item => item.toolDisplayName)
        return list.join(', ')
      },
    },
  }
