  ...
  <div class="main-content-inner main-content-form" :class="editDisabled ? 'from-pipeline' : ''">
    <div v-if="editDisabled" :model="formData">
      <div class="to-pipeline">
        <span v-if="projectId === 'CUSTOMPROJ_PCG_RD'">{{$t('基础信息配置由PCG EP度量平台自动生成')}}</span>
        <span v-else-if="taskDetail.createFrom === 'gongfeng_scan' || taskDetail.createFrom === 'api_trigger'">{{$t('基础信息配置由CodeCC开源扫描集群自动生成')}}</span>
