<template>
  <div>
    <div v-if="!isEditable || projectId === 'CUSTOMPROJ_PCG_RD'" class="from-pipeline">
      <div class="to-pipeline">
        <span v-if="projectId === 'CUSTOMPROJ_PCG_RD'">{{$t('扫描触发配置由PCG EP度量平台自动生成')}}</span>
        <span v-else>{{$t('修改扫描触发配置，请前往流水线')}} <a @click="hanldeToPipeline" href="javascript:;">{{$t('立即前往>>')}}</a></span>
      </div>
      <div>
        <span class="pipeline-label">{{$t('扫描方式')}}
          <bk-popover>
            ...
            <div slot="content">
              <div>{{$t('快速全量扫描：首次会扫描全部文件。后续扫描未变更文件会复用之前的扫描结果，提升扫描速度。')}}</div>
        ...
        <span>
          <span class="fs14" v-if="taskDetail.scanType === 1">{{ $t('快速全量扫描') }}</span>
          <span class="fs14" v-else-if="taskDetail.scanType === 0">{{$t('全量扫描')}}</span>
          <span class="fs14" v-else-if="taskDetail.scanType === 6">{{$t('差异扫描')}}</span>
          <span class="fs14" v-else-if="taskDetail.scanType === 2 || taskDetail.scanType === 5">{{$t('MR/PR扫描')}}</span>
        </span>
      </div>
      <div>
      ...
      <div>
        ...
        <div v-if="authorList" class="handler-replace">
          <div v-for="(item, index) in authorList" :key="index">
            ...
            <span class="fs14 ml10" v-if="item.targetAuthor.join()">{{$t('(原处理人 ') + item.sourceAuthor + ')'}}</span>
          </div>
        </div>
      </div>
      <div>
        ...
        <span>
          <span class="fs14" v-if="taskDetail.prohibitIgnore">{{ $t('禁止页面忽略') }}</span>
          ...
          <div class="settings-body">
            <bk-form-item :label-width="110">
              <bk-radio-group v-model="taskDetail.scanType">
                <bk-radio :value="1" class="pr30">{{$t('快速全量扫描')}}</bk-radio>
                <bk-radio :value="0" class="pr30">{{$t('全量扫描')}}</bk-radio>
              </bk-radio-group>
            </bk-form-item>
          </div>
          <div class="settings-header">
        ...
        <div class="settings-body">
          <bk-form-item :label-width="110">
            <bk-radio-group v-model="prohibitIgnore">
              <bk-radio :value="false" class="pr30">{{$t('允许页面忽略')}}</bk-radio>
  ...
  export default {
    ...
    methods: {
      init() {
        this.$store.dispatch('defect/getTransferAuthorList').then((res) => {
          ...
          this.authorList = res.transferAuthorList
            ? this.fomatterAuthor(res.transferAuthorList, 1) : [{ sourceAuthor: '', targetAuthor: [] }]
          this.date = this.fromDate
        })
      },
      saveTime() {
        const { taskId } = this.$route.params
        const { prohibitIgnore } = this
        ...
        const data = { taskId, scanType, prohibitIgnore,
                       timeAnalysisConfig: { executeDate, executeTime }, newDefectJudge }
      ...
      fomatterAuthor(authorObj, type) {
        return authorObj.map(item => ({
          ...
          targetAuthor: item.targetAuthor
            ? type === 1 ? item.targetAuthor.split(',') : item.targetAuthor.join(',')
            : [],
        }))
      },
      ...
      ableHandlerConversion() {
        if (this.authorList.length === 1 && this.authorList[0].sourceAuthor === ''
