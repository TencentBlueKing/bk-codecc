  ...
  <div>
    <div v-if="taskDetail.createFrom === 'gongfeng_scan'">
      <div class="no-authority">
        ...
        <bk-button size="large" theme="primary" @click="hanldeToGongFeng()">{{$t('前往工蜂成员管理')}}</bk-button>
      </div>
    </div>
    <bk-tab v-else class="settings-authority-tab" :label-height="42" :active.sync="active" type="unborder-card" @tab-change="changeTab">
          ...
          <bk-collapse class="collapse" v-for="item in policyList" :key="item" v-model="funcActive">
            <bk-collapse-item :name="item.policy_code">
              <div>
                ...
                <span v-if="item.policy_code === 'analyze'">{{$t('(触发立即检查)')}}</span>
          ...
          .then((res) => {
            ...
            if (this.roleList) {
              [this.roleList[0], this.roleList[1]] = this.roleList[0].role_code === 'executor'
                ? [this.roleList[1], this.roleList[0]] : [this.roleList[0], this.roleList[1]]
            }
            this.roleList.forEach((role) => {
              role.group_list = role.group_list.map(grouplist => grouplist.group_id)
            })
      ...
      saveUserSetting() {
        ...
        try {
          ...
          axios.put(
            ...
            data,
            { withCredentials: true,
              headers: { 'X-CSRFToken': cookie.parse(document.cookie).paas_perm_csrftoken },
            },
          )
            .then((res) => {
        ...
        ['policy', 'role'].forEach((item) => {
          ...
          list.forEach((user) => {
            ...
            if (user.role_code === 'manager') {
              taskOwner = Array.from(new Set(taskOwner.concat(user_list).concat(extra_user_list)
                .concat(groupList)
                .concat(extraGroupList)))
            } else {
              taskMember = Array.from(new Set(taskMember.concat(user_list).concat(extra_user_list)
                .concat(groupList)
                .concat(extraGroupList)))
            }
          })
