<template>
  <div class="main-content-inner checker">
    <span
      ...
      "
      class="fs12 go-pipeline"
    >{{ $t('规则集配置由CodeCC开源扫描集群自动生成') }}</span
    >
    <span v-else-if="editDisabled" class="fs12 go-pipeline">
      {{ $t('规则集为x。如需修改，请前往流水线', { name: checkersetStr }) }}
      <a class="ml15" @click="handleToPipeline" href="javascript:;">{{
        $t('立即前往>>')
      }}</a>
    </span>
    <div v-if="checkersetList.length">
      <p class="checker-title">
        ...
        <i
          class="codecc-icon icon-tips"
          v-bk-tooltips="{
            content: $t(
              '以下规则集适合于当前任务语言。新启用的规则集将在下次检查时生效'
            ),
          }"
        ></i>
        <a
    ...
    <bk-dialog
      v-model="checkersetDialogVisible"
      :render-directive="'if'"
      :fullscreen="true"
      :draggable="false"
      :mask-close="false"
    ...
    >
      <div class="checkerset-dialog-main">
        <checkerset-manage
          ...
          @closeDialog="checkersetDialogVisible = false"
        >
        </checkerset-manage>
      </div>
    </bk-dialog>
    <bk-dialog
      v-model="checkersetListDialogVisible"
      :render-directive="'if'"
      :fullscreen="true"
      :draggable="false"
      :mask-close="false"
...
export default {
  ...
  data() {
    return {
      isFetched: false,
      checkersetList: [],
      checkersetDialogVisible: false,
      checkersetListDialogVisible: false,
      checkersetId: '',
      version: '',
      checkerSetEnvData: {},
    };
  ...
  computed: {
    ...
    checkersetStr() {
      ...
      const preProdKey = `preProd${checkerSetType.replaceAll(
        '_',
        '',
      )}TimeGap`.toLocaleLowerCase();
      const prodKey = `prod${checkerSetType.replaceAll(
        '_',
        '',
      )}TimeGap`.toLocaleLowerCase();
      ...
      const ProdTime = this.formatTime(checkerSetEnvData[prodTimeKey]
          && checkerSetEnvData[prodTimeKey].startTime);
      const strMap = {
  ...
  methods: {
    ...
    handleCheckerset(checkerset) {
      ...
      this.checkersetDialogVisible = true;
    },
    handleToPipeline() {
...
${this.taskDetail.pipelineId}/edit#${this.taskDetail.atomCode}`,
    ...
    hasCcnTips(checkerset) {
      ...
      return (
        isFromCcndupc && isNotPipeline && ownList && ownList[0] === checkerset
      );
    },
    ...
    updateCheckerList() {
      this.fetchList();
      this.checkersetDialogVisible = false;
    },
    formatTime(time) {
