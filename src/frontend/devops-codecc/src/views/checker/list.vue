  ...
  <div class="cc-checkers">
    ...
    <div class="cc-main" v-bkloading="{ isLoading: pageLoading, opacity: 0.6 }">
      ...
      <span class="table-txt" v-else>
        <span class="table-txt-num">{{$t('第 x / y 规则', { num: tableIndex + 1, count: totalCount })}}</span>
      ...
      <bk-table
        ...
        <bk-table-column :label="$t('适用语言')" prop="checkerLanguage" sortable="custom">
          <template slot-scope="props">
            <span>{{props.row.checkerLanguage && props.row.checkerLanguage.join()}}</span>
          </template>
        </bk-table-column>
        <bk-table-column :label="$t('类别')" prop="checkerCategory" sortable="custom">
          <template slot-scope="props">
    ...
    <bk-sideslider :transfer="true" :is-show.sync="sliderIsShow" :quick-close="true" width="720">
      ...
      <div slot="content" class="checker-content" v-bkloading="{ isLoading: contentLoading, opacity: 0.6 }">
        <div class="checker-form">
          ...
          <span class="checker-col">
            <span class="checker-col-label label-fixed">{{$t('标签：')}}</span>
          ...
          <span class="checker-col" v-if="checker.editable && checker.props">
            ...
            <span class="checker-col-content">
              <div v-for="item in checker.propsArr" :key="item" class="checker-props">{{item.propName}} = {{item.propValue}}</div>
              <i class="codecc-icon icon-edit" @click.stop="handleCheckerEdit(checker, true)"></i>
            </span>
          </span>
          <span class="checker-col checker-col-block">
            <span class="checker-col-label">{{$t('描述：')}}</span>
            <span class="checker-col-content">
              <div v-for="(desc, index) in (checker.checkerDesc && checker.checkerDesc.split('\n'))" :key="index">
        ...
        <bk-form :label-width="120" ref="ruleParameter">
          ...
          <bk-form-item :label="$t('参数修改')" class="params-input" property="propValue">
            <div class="pb10" v-for="item in ruleData.propsArr" :key="item">
              ...
              <bk-input style="width: 150px" :disabled="!isConfig" v-model="item.propValue"></bk-input>
            </div>
  ...
  export default {
    ...
    methods: {
      async fetchSearch(isInit, params = {}) {
        const payload = this.isConfig ? Object.assign(
        ...
        if (res) {
          if (this.isConfig) {
            const filterArr = this.checkersetConf.legacy
              && this.checkersetConf.codeLangList.length === 1 ? ['checkerLanguage', 'toolName'] : ['checkerLanguage']
      ...
      async fetchList(selectParams = this.selectParams) {
        ...
        const params = this.isConfig
          ? Object.assign({ checkerLanguage: this.checkersetConf.codeLang }, selectParams)
          : selectParams
        if (this.isConfig) {
          ...
          if (list.length) {
            ...
            this.selectedConf.forEach((item) => {
              const target = this.list
                .filter(val => val.toolName === item.toolName && val.checkerKey === item.checkerKey)
      ...
      handleRowClick(row = this.list[this.tableIndex], index = this.tableIndex) {
        ...
        this.$store.dispatch('defect/getWarnContent', { toolName: row.toolName, checkerKey }).then((res) => {
          if (res.errExample) {
      ...
      selectRules(row, checker) {
        ...
        const isChecked = validArr
          .filter(item => (item.checkerKey === checker.checkerKey) && (item.toolName === checker.toolName)).length
