<template>
  <div>
    <bk-sideslider :transfer="true" class="create-checkerset-wrapper" :title="title" width="560" :is-show.sync="visiable" @hidden="closeSlider">
      <div class="slider-content" slot="content">
        <bk-form class="checkerset-side-form" ref="checkersetForm" v-if="visiable" :model="newCheckersetParams" label-width="100">
          ...
          <bk-form-item :label="$t('描述')" :rules="formRules.description" :required="true" property="description">
            <bk-input type="textarea" v-model="newCheckersetParams.description" :placeholder="$t('请输入描述')"></bk-input>
          </bk-form-item>
          ...
          <template v-if="hasDetail">
            <bk-form-item :desc="$t('发布者创建规则集所在项目')" :label="$t('源项目')">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      isListRouter() {
        return this.$route.name === 'checkerset-list' || this.$route.name === 'task-settings-checkerset'
      },
      title() {
    ...
    watch: {
      ...
      'newCheckersetParams.projectId'(newVal) {
        axios
          .get(
            `${window.DEVOPS_API_URL}/project/api/user/projects/${newVal}`,
