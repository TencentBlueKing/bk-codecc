  ...
  <div class="checkerset-card" :class="{ 'enable': isEnable || isFlash, 'disable': from === 'task' && !isEnable }">
    ...
    <section class="ruleset-info">
      <p class="ruleset-name">
        ...
        <span :title="$t('复制规则集ID')" class="copy-icon" @click="copyCheckerset(checkerset.checkerSetId)"><i class="codecc-icon icon-copy-line"></i></span>
      </p>
...
<style lang="postcss">
    ...
    .checkerset-card {
      ...
      .use-mark {
        ...
        border-radius: 2px;
        &.preferred {
          background-color: rgba(134, 223, 38, .3);
          color: rgba(53, 99, 22, .8);
          border: 1px solid rgba(102, 197, 1, .3);
        }
        &.recommend {
          background-color: rgba(211, 224, 255, .3);
          color: rgba(61, 76, 138, .8);
          border: 1px solid rgba(187, 204, 244, .3);
        }
      }
      .ruleset-id {
        overflow: hidden;
