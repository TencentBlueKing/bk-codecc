<template>
  <div class="checkerset-manage">
    ...
    <div class="checkerset-info" v-if="detailInfo.checkerSetId">
      <span class="breadcrumb-txt" @click="back"><i class="bk-icon icon-angle-left"></i>{{$t('返回')}}</span>
    ...
    methods: {
      ...
      handleSelectRules(data, isChecked, isBatch) {
        if (isBatch) {
          ...
          } else validArr.forEach(val => this.selectedRuleList
            .splice(this.selectedRuleList
              .findIndex(item => item.checkerKey === val.checkerKey && item.toolName === val.toolName), 1))
        } else {
          if (isChecked) {
            this.selectedRuleList.push(data)
          } else this.selectedRuleList
            .splice(this.selectedRuleList
              .findIndex(item => item.checkerKey === data.checkerKey && item.toolName === data.toolName), 1)
        }
      },
      async submit() {
        const version = this.version || this.$route.params.version
