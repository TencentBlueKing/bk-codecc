    ...
    <div class="main-container">
      ...
      <div class="authors-wrapper">
        <div class="authors-charts">
          <div
            class="authors-charts-item"
            id="existAuthorsChart"
            ref="existAuthorsChart"
          ></div>
        </div>
              ...
              <bk-table-column :label="$t('总数')" prop="total" align="center">
                <template slot-scope="{ row }">
                  <a
                    ...
                    href="javascript:;"
                    @click="handleHref({ author: row.authorName })"
                  >{{ row.total }}</a
                  >
                  <a v-else href="javascript:;" @click="handleHref({})">{{
                    row.total
                  }}</a>
                </template>
              </bk-table-column>
              ...
              >
                <template slot-scope="{ row }">
                  <a
                    v-if="row.authorName !== 'Total'"
                    href="javascript:;"
                    @click="handleHref({ author: row.authorName, severity: 1 })"
                  >{{ row.serious }}</a
                  >
                  <a
                    v-else
                    href="javascript:;"
                    @click="handleHref({ severity: 1 })"
                  >{{ row.serious }}</a
                  >
                </template>
              </bk-table-column>
              <bk-table-column :label="$t('一般')" prop="normal" align="center">
                <template slot-scope="{ row }">
                  <a
                    v-if="row.authorName !== 'Total'"
                    href="javascript:;"
                    @click="handleHref({ author: row.authorName, severity: 2 })"
                  >{{ row.normal }}</a
                  >
                  <a
                    v-else
                    href="javascript:;"
                    @click="handleHref({ severity: 2 })"
                  >{{ row.normal }}</a
                  >
                </template>
              </bk-table-column>
              <bk-table-column :label="$t('提示')" prop="prompt" align="center">
                <template slot-scope="{ row }">
                  <a
                    v-if="row.authorName !== 'Total'"
                    href="javascript:;"
                    @click="handleHref({ author: row.authorName, severity: 4 })"
                  >{{ row.prompt }}</a
                  >
                  <a
                    v-else
                    href="javascript:;"
                    @click="handleHref({ severity: 4 })"
                  >{{ row.prompt }}</a
                  >
                </template>
              </bk-table-column>
              ...
              >
                ...
                <bk-table-column :label="$t('待修复数')" align="center">
                  <template slot-scope="{ row }">
                    <a
                      @click="
                        handleHref({ startTime: row.date, endTime: row.date })
                      "
                      href="javascript:;"
                    >{{ row.existCount }}</a
                    >
    ...
    searchParams: {
      handler() {
        this.fetchLintList().then((res) => {
          this.initTrend(
            'exist',
            res.newCloseFixChart && res.newCloseFixChart.elemList,
          ); // 待修复
  ...
  methods: {
    ...
    initTrend(trendType, list = []) {
      ...
      if (trendTypeMap[trendType].name2) {
        legend = [
          { name: trendTypeMap[trendType].name },
          { name: trendTypeMap[trendType].name2 },
        ];
        series = [
          {
            name: trendTypeMap[trendType].name,
    ...
    downloadExcel() {
      ...
      const excelData2 = this.getExcelData(
        [this.$t('日期'), this.$t('新增数'), this.$t('待修复数')],
        ['tips', 'newCount', 'existCount'],
        this.trendNewTableData,
        this.$t('每日新增问题'),
      );
