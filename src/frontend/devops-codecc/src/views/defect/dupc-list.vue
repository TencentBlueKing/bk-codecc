<template>
  ...
  >
    <div
      class="dupc-list"
      v-if="taskDetail.enableToolList.find((item) => item.toolName === 'DUPC')"
    >
          ...
          >
            ...
            <bk-table-column :label="$t('文件名')" prop="fileName">
              <template slot-scope="props">
                <span
                  v-bk-tooltips="{ content: props.row.filePath, delay: 600 }"
            ...
            <bk-table-column
              ...
              prop="dupLines"
              sortable="custom"
              label-class-name="col-sort-label"
              class-name="col-sort"
            ></bk-table-column>
            <bk-table-column
              :label="$t('总行数')"
            ...
            >
              <template slot-scope="props">
                <span
                  :class="`color-${
                    { 1: 'major', 2: 'minor', 4: 'info' }[props.row.riskFactor]
                  }`"
            ...
            >
              <template slot-scope="props">
                <span>{{
                  formatTime(props.row.fileChangeTime, 'yyyy-MM-dd')
                }}</span>
              </template>
            </bk-table-column>
            <div slot="empty">
      ...
      <div class="main-container large border-none">
        <div class="no-task">
          <empty
            ...
            :desc="
              $t(
                'CodeCC集成了重复率工具，可以发现冗余和重复代码，以便代码抽象和重构'
              )
            "
          >
    ...
    <bk-dialog
      ...
      :show-footer="false"
    >
      <dupc-detail :entity-id="entityId" :file-path="filePath"></dupc-detail>
    </bk-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    this.getCustomOption = function (val) {
      return [{ id: 'severity', name: this.$t('风险级别'), isChecked: val }];
    };
  ...
  created() {
    if (
      ...
      this.taskDetail.enableToolList.find((item) => item.toolName === 'DUPC')
    ) {
      this.init();
    }

    // 读取缓存搜索过滤项
    const columnsCache = JSON.parse(
      localStorage.getItem(DUPC_SEARCH_OPTION_CACHE)
    );
    if (columnsCache) {
  ...
  methods: {
    ...
    handleListRowClick(row, event, column) {
      this.fileIndex = this.defectList.findIndex(
        (file) => file.entityId === row.entityId
      );
      ...
      function keyDown(event) {
        ...
        switch (e.code) {
          case 'Enter': // enter
            ...
            if (!vm.detailVisible && e.path.length < 5) vm.keyEnter();
            break;
          case 'Escape': // esc
    ...
    downloadExcel() {
      ...
      if (this.totalCount > 300000) {
        this.$bkMessage({
          message: this.$t(
            '当前问题数已超过30万个，无法直接导出excel，请筛选后再尝试导出。'
          ),
        });
        return;
      }
    ...
    generateExcel(list = []) {
      ...
      const title = `${this.taskDetail.nameCn}-${this.taskDetail.taskId}-${
        this.toolId
      }-${this.$t('重复文件')}-${new Date().toISOString()}`;
