    ...
    <section
      ...
      v-if="
        taskDetail.enableToolList.find(
          (item) => item.toolName !== 'CCN' && item.toolName !== 'DUPC'
        ) ||
          isFromOverview ||
          isProjectDefect
      "
    >
      <div class="breadcrumb">
        <div class="breadcrumb-name">
          ...
          <chart-panel v-else :tool-name="toolNameStr" :tool-list="toolList" />
          <div
          ...
          >
            ...
            <span
              class="filter-collapse-icon mr20"
              :class="{ 'mac-filter-search-icon': isMac }"
            >
              <bk-popover
                ext-cls="handle-menu"
                ref="handleMenu"
      ...
      <div class="main-container" ref="mainContainer">
        <div class="main-content-inner main-content-list">
            ...
            >
              <container class="cc-container">
                ...
                <div
                  class="cc-col"
                  v-if="!isProjectDefect"
                  v-show="allRenderColumnMap.path"
                >
                ...
                <div class="cc-col" v-show="allRenderColumnMap.approval && isInnerSite">
                  <bk-form-item :label="$t('审批')">
                    <bk-select
                      ...
                      searchable
                      multiple
                      v-model="searchParams.ignoreApprovalStatus"
                      :key="approvalTreeKey"
                      :remote-method="handleApprovalRemote"
                      :tag-fixed-height="false"
                      :loading="selectLoading.approvalLoading"
                      @tab-remove="handleApprovalValuesChange"
                      @clear="handleApprovalClear"
                    >
                      <bk-big-tree
                        ...
                        class="tree-select"
                        ref="approvalTree"
                        :default-checked-nodes="searchParams.ignoreApprovalStatus"
                        @check-change="handleApprovalCheckChange"
                      >
                        <div slot-scope="{ data }">
                          <div>{{ data.name }}</div>
                        </div>
                ...
                <div class="cc-col" v-show="allRenderColumnMap.operate">
                  <bk-form-item :label="$t('操作')">
                    <bk-select
                      ref="operateSelect"
                      searchable
                      multiple
                      v-model="searchParams.operates"
                      :key="operateTreeKey"
                      :remote-method="handleOperateRemote"
                      :tag-fixed-height="false"
                      :loading="selectLoading.operateLoading"
                      @tab-remove="handleOperateValuesChange"
                      @clear="handleOperateClear"
                    >
                      <bk-big-tree
                        ...
                        class="tree-select"
                        ref="operateTree"
                        :default-checked-nodes="searchParams.operates"
                        @check-change="handleOperateCheckChange"
                      >
                        <div slot-scope="{ data }">
                          <div>{{ data.name }}</div>
                        </div>
          ...
          <div
            class="cc-table"
            :style="isSearchDropdown ? '' : 'z-index: 100'"
            ref="ccTable"
            v-bkloading="{
          ...
          >
            <div class="cc-selected">
              ...
              <span v-else>{{
                $t('已选择x问题，共y问题', {
                  x: isSelectAll === 'Y' ? totalCount : selectedLen,
                  y: totalCount,
                })
              }}</span>
              <span
                v-if="gatherFile.fileCount"
                class="extra-file"
                @click="goToLog"
              >
                {{
                  $t('另有x个大文件y问题', {
                    x: gatherFile.fileCount,
                    y: gatherFile.defectCount,
                  })
                }}
                <bk-popover placement="top" width="300" class="popover">
                  ...
                  <div slot="content">
                    {{
                      $t(
                        '对于部分大文件产生的海量告警，CodeCC已将其归档。可前往工具分析记录下载文件查看详情'
                      )
                    }}
                  </div>
                </bk-popover>
            ...
            <div v-if="isBatchOperationShow" class="cc-operate pb10">
              <div class="cc-operate-buttons">
                ...
                <bk-button
                  size="small"
                  ext-cls="cc-operate-button"
                  v-if="searchParams.clusterType === 'defect'"
                ...
                >
                  ...
                  <div class="handle-menu-tips" slot="dropdown-content">
                    ...
                    <p
                      ...
                      @click.stop="handleIgnore('IgnoreApproval|IgnoreDefect', true)"
                      :title="
                        $t(
      ...
      >
        <div class="reason-type-list">
          ...
          <bk-form
            ...
            :label-width="0"
            class="search-form"
            v-bkloading="{ isLoading: preIgnoreApprovalLoading, opacity: 0.3 }">
              ...
              >
                <div v-for="ignore in ignoreList.filter(i => i.status !== 2)" :key="ignore.ignoreTypeId">
                  <bk-radio
                    ...
                    @change="() => handleChangeIgnoreReasonType(ignore)"
                  >
                    {{ ignore.name }}
                  ...
                  <FoldAlert
                    v-if="operateParams.ignoreReasonType === ignore.ignoreTypeId"
...
export default {
  ...
  data() {
    ...
    this.getCustomOption = val => (isProjectDefect
      ? [
    ...
    return {
      ...
      searchParams: {
        ...
        ignoreApprovalId: query.ignoreApprovalId || '',
        operate,
        operates,
  ...
  computed: {
    ...
    operateTreeData() {
      ...
      const operateList = [
        ...innerOperateList,
        {
          id: 'MARK',
          name: `${this.$t('已标记处理')}${
            isProjectDefect ? '' : `(${maskOpsCount || 0})`
          }`,
        },
        {
          id: 'MARK_NOT_FIXED',
          name: `${this.$t('标记处理后扫描仍为问题')}${
            isProjectDefect ? '' : `(${maskNotFixCount || 0})`
          }`,
        },
        {
          id: 'COMMENT',
          name: `${this.$t('已评论')}${
            isProjectDefect ? '' : `(${commentOpsCount || 0})`
          }`,
        },
        {
          id: 'NO_OPS',
          name: `${this.$t('无操作')}${
            isProjectDefect ? '' : `(${notOpsCount || 0})`
          }`,
        },
      ];
      this.operateTreeKey += 1;
      return operateList;
    },
    nextPageStartNum() {
    ...
    allCheckerList() {
      const { checkerList = [] } = this.searchFormData;
      return checkerList.reduce((prev, next) => prev.concat(next.checkers), [])
  ...
  watch: {
    ...
    searchParamsWatch: {
      handler(newVal, oldVal) {
        ...
        if (
          newVal.clusterType === 'file'
          && !(newVal.status.length === 1 && newVal.status[0] === 1)
        ) {
          return;
        }
        if (this.isSearch) {
    ...
    defectDetailSearchParams: {
      handler(val) {
        ...
        this.defectDetailDialogVisible = this.isShowDetailDialog;
    ...
    'searchParams.clusterType'(newVal) {
      ...
      if (
        newVal === 'file'
        && !(
          this.searchParams.status.length === 1
          && this.searchParams.status[0] === 1
        )
      ) {
    ...
    'searchParams.operate'(val, oldVal) {
      if (!_.isEqual(val, oldVal)) {
    ...
    'searchParams.ignoreReasonTypes'(val, oldVal) {
      if (!_.isEqual(val, oldVal)) {
  ...
  created() {
    if (
      ...
      || this.taskDetail.enableToolList.find(item => item.toolName !== 'CCN' && item.toolName !== 'DUPC')
    ) {
      this.init(true);
      this.fetchOtherParams();
  ...
  methods: {
    ...
    generateExcel(list = [], clusterType) {
      ...
      const prefix = isProjectDefect
        ...
        : `${this.taskDetail.nameCn}-${this.taskDetail.taskId}-${this.dimensionStr}-`;
    ...
    formatJson(filterVal, list, clusterType) {
      ...
      return list.map(item => filterVal.map((j) => {
        ...
        if (j === 'severityList') {
          return (
            item[j]
              && item[j].map(i => this.defectSeverityMap[i]).join('、')
          );
        }
    ...
    async init(isInit) {
      ...
      this.addTableScrollEvent();
      if (isInit) {
        this.handleGatherFile(); // clusterType === 'file' 时触发
        this.fetchCheckerSetParams(); // 获取规则集列表
    ...
    async fetchSeverityParams() {
      ...
      this.newDefectJudgeTime = newDefectJudgeTime
        ? this.formatTime(newDefectJudgeTime, 'yyyy-MM-dd')
        : '';
    ...
    async fetchOperateParams() {
      this.selectLoading.operateLoading = true;

      const params = this.getSearchParams();
      params.statisticType = 'OPERATE';
      const res = await this.$store.dispatch('defect/lintSearchParams', params);
      const { tapdOpsCount, maskOpsCount, maskNotFixCount, commentOpsCount, notOpsCount } = res;
      this.searchFormData = Object.assign(this.searchFormData, {
        tapdOpsCount,
        maskOpsCount,
        maskNotFixCount,
      ...
      this.selectLoading.operateLoading = false;
    },
    ...
    async handleGatherFile() {
      ...
      this.gatherFile = (await this.$store.dispatch('defect/gatherFile', {
        taskIdList,
        toolNameList: toolName,
        dimensionList: dimension,
        buildId,
    ...
    fetchLintDetail(type, extraParams = {}) {
      ...
      const api = toolList.includes(toolName) ? 'defect/lintDetail' : 'defect/lintDetailWithoutContent';
      this.$store
        .dispatch(api, params)
        .then((detail) => {
    ...
    getDefectCountByType(type) {
      ...
      const count = this.searchFormData[typeFieldMap[type]] || 0;
      return count > 100000 ? this.$t('10万+') : count;
    },
    ...
    handleComment(entityId, commentId) {
      this.commentParams = {
        ...this.commentParams,
        defectId: entityId,
        comment: '',
      ...
      if (commentId) {
        ...
        this.commentParams.commentId = this.lintDetail.lintDefectDetailVO
          .codeComment
    ...
    handleCommentConfirm() {
      ...
      this.$store
        .dispatch(url, this.commentParams)
        .then((res) => {
          if (res.code === '0') {
            this.$bkMessage({
              theme: 'success',
    ...
    handleIgnoreConfirm() {
      ...
      if (this.isSelectAll === 'Y') {
        ...
        && !this.operateParams.defectKeySet[0]
      ) {
        ...
        data = {
          ...data,
          isSelectAll: 'Y',
          queryDefectCondition: JSON.stringify(searchParams),
        };
        data.defectKeySet = [];
      }
      const {
              ...
              if (index < 0) {
                ...
                this.listData.defectList.records.forEach((item) => {
                  if (item.entityId === data.defectKeySet[0]) {
                    ...
                    } else if (this.operateParams.bizType === 'IgnoreApproval|IgnoreDefect') {
                      if (isIgnoreApproval) {
