    ...
    <div
      ...
      v-if="
        taskDetail.enableToolList.find(
          (item) => item.toolName === 'SCC' || item.toolName === 'CLOC'
        )
      "
    >
            ...
            <template v-for="(item, index) in pathList">
              <template v-if="item.isRoot">
                <bk-popover
                  placement="top"
                  :key="index"
                  v-if="codeRepo.length > 1"
                >
                  <span
                    v-if="item.isPath"
                    class="mr5 cc-link-primary project-name"
          ...
          >
            ...
            >
              <template slot-scope="props">
                <i
                  class="codecc-icon mr5 f14"
                  :class="[
                    props.row.clocChildren
                      ? 'icon-folders'
                      : props.row.type !== 'sum'
                        ? 'icon-file'
                        : '',
                ...
                ></i>
                <span
                  :class="{ 'cloc-link': props.row.clocChildren }"
                  @click="openFolder(props.row.name, !!props.row.clocChildren)"
                >
                  {{ props.row.name }}
                </span>
              </template>
              ...
              <template slot-scope="props">
                <span
                  v-bk-tooltips="{
                    content:
                      $t('代码行比率') +
                      handleRate(props.row.codeLines, props.row.totalLines),
                  }"
                >
                  {{ props.row.codeLines }}
                </span>
            ...
            >
              <template slot-scope="props">
                <span
                  v-if="
                    realTool === 'SCC' &&
                      props.row.efficientCommentLines !== undefined
                  "
                  v-bk-tooltips="{
                    content:
                      $t('注释率') +
                      handleCommentRate(props.row.efficientCommentRate),
                  }"
                >
              ...
              <template slot-scope="props">
                <span
                  v-bk-tooltips="{
                    content:
                      $t('空白率') +
                      handleRate(props.row.blankLines, props.row.totalLines),
                  }"
                >
      ...
      <div class="main-container large border-none">
        <div class="no-task">
          <empty
            ...
            :desc="
              $t(
                'CodeCC集成了代码统计工具，可以检测代码中所包含的语言种类，以及代码行、注释行、空白行等占比'
              )
            "
          >
            <template #action>
...
export default {
  ...
  methods: {
    async init(isInit) {
      ...
      const res = await this.$store.dispatch('defect/lintListCloc', {
        toolId: this.toolId,
        type: 'FILE',
      });
      if (isInit) {
      ...
      if (res) {
        ...
        this.clocTree = (res.clocTreeNodeVO
            && res.clocTreeNodeVO.clocChildren
            && res.clocTreeNodeVO.clocChildren[0])
          || {};
...
/${that.taskDetail.pipelineId}/edit#${that.taskDetail.atomCode}`,
    ...
    renderHeader(h, data) {
      const id = '820084513';
      const hrefDom = (DEPLOY_ENV === 'tencent'
        ? `<a target="_blank" href="${window.IWIKI_SITE_URL}/${id}">${this.$t('查看详细说明')}</a>`
        : '');
      const directive = {
        name: 'bkTooltips',
      ...
      return (
        <span class="custom-header-cell" v-bk-tooltips={directive}>
          {data.column.label}
        </span>
      );
    },
