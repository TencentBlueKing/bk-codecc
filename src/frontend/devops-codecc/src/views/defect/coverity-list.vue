<template>
  ...
  >
    <section
      class="coverity-list"
      v-if="
        taskDetail.enableToolList.find(
          (item) =>
            item.toolName === 'COVERITY' ||
            item.toolName === 'KLOCWORK' ||
            item.toolName === 'PINPOINT'
        ) || isFromOverview
      "
    >
      <div class="breadcrumb">
        <div class="breadcrumb-name">
          ...
          >
            ...
            >
              <i
                class="bk-icon codecc-icon icon-shaixuan"
      ...
      <div class="main-container" ref="mainContainer">
        <div class="main-content-inner main-content-list">
            ...
            >
              <container class="cc-container">
                ...
                <div
                  class="cc-col"
                  v-show="
                    allRenderColumnMap.checker &&
                      (lineAverageOpt >= 4 || isSearchDropdown)
                  "
                >
                  <bk-form-item :label="$t('规则')">
                    <bk-select
                      v-model="searchParams.checker"
                      searchable
                ...
                <div
                  class="cc-col"
                  v-show="
                    allRenderColumnMap.author &&
                      (lineAverageOpt >= 5 || isSearchDropdown)
                  "
                >
                ...
                <div
                  class="cc-col"
                  v-show="
                    allRenderColumnMap.daterange &&
                      (lineAverageOpt >= 6 || isSearchDropdown)
                  "
                >
                ...
                <div
                  class="cc-col"
                  v-show="
                    allRenderColumnMap.path &&
                      (lineAverageOpt >= 7 || isSearchDropdown)
                  "
                >
                  <bk-form-item :label="$t('路径')">
                    <bk-select
                      v-if="selectLoading.otherParamsLoading"
                      :loading="true"
                    ></bk-select>
                    <bk-dropdown-menu
                      v-else
                ...
                <div
                  class="cc-col"
                  v-show="
                    allRenderColumnMap.status &&
                      (lineAverageOpt >= 8 || isSearchDropdown)
                  "
                >
                ...
                <div
                  class="cc-col"
                  v-show="
                    allRenderColumnMap.ignoreType &&
                      (lineAverageOpt >= 9 || isSearchDropdown)
                  "
                >
                  <bk-form-item :label="$t('忽略类型')">
                    ...
                    >
                      <bk-option
                        ...
                        :class="{
                          'guide-item':
                            ignore.ignoreTypeId === 24 && !guideFlag,
                        }"
                      >
                ...
                <div
                  class="cc-col"
                  v-show="
                    allRenderColumnMap.severity &&
                      (lineAverageOpt >= 10 || isSearchDropdown)
                  "
                >
                  <bk-form-item :label="$t('级别')">
                    ...
                    >
                      ...
                      >
                        ...
                        >{{ getDefectCountBySeverity(key) }}</em
                        >)
                      </bk-checkbox>
                    </bk-checkbox-group>
                  </bk-form-item>
          ...
          <div
            ...
            ref="ccTable"
            v-bkloading="{
              isLoading: tableLoading && !defectDetailDialogVisible,
              opacity: 0.6,
            }"
          >
            ...
            <p class="search-more-option" v-if="!isBatchOperationShow">
              <i
                :class="[
                  'bk-icon codecc-icon icon-codecc-arrow',
                  { 'icon-flip': isSearchDropdown },
                ]"
            ...
            <div v-if="isBatchOperationShow" class="cc-operate pb10">
              <div class="cc-operate-buttons">
                ...
                <bk-button
                  size="small"
                  ext-cls="cc-operate-button"
                  @click="handleIgnore('IgnoreDefect', true)"
                  theme="primary"
                >{{ $t('忽略') }}</bk-button
                >
                <bk-button
            ...
            >
              ...
              <bk-table-column width="15" class-name="mark-row">
                <template slot-scope="props">
                  <span
                    v-if="props.row.status === 1 && props.row.mark"
                    class="cc-icon-mark"
                  ></span>
                </template>
              </bk-table-column>
              ...
              >
                <template slot-scope="props">
                  <span v-bk-tooltips="props.row.filePath">{{
                    props.row.fileName
                  }}</span>
                </template>
              </bk-table-column>
              ...
              <bk-table-column :label="$t('问题描述')" prop="message">
                <template slot-scope="props">
                  <span :title="props.row.message || props.row.displayType">{{
                    props.row.message || props.row.displayType
                  }}</span>
                </template>
              </bk-table-column>
              <bk-table-column
                  ...
                  <div
                    ...
                    @click.stop="
                      handleAuthor(
                        1,
                        props.row.entityId,
                        props.row.authorList && props.row.authorList
                      )
                    "
                  >
                    <span>{{
              ...
              <bk-table-column
                ...
                :formatter="
                  (row, column, cellValue, index) =>
                    formatTime(cellValue, 'yyyy-MM-dd')
                "
              >
              </bk-table-column>
              <bk-table-column
              ...
              <bk-table-column :label="$t('状态')" prop="status" width="110">
                <template slot-scope="props">
                  <span>{{
                    handleStatus(props.row.status, props.row.defectIssueInfoVO)
                  }}</span>
                </template>
              </bk-table-column>
                    ...
                    >
                      ...
                      <div slot="content" class="handle-menu-tips txal">
                        ...
                        <p
                          ...
                          class="entry-link"
                          @click="handleRevertIgnoreAndMark(props.row.entityId)"
                        >
                          {{ $t('取消忽略并标记处理') }}
                        </p>
                        <p
          ...
          >
            <div
              v-bkloading="{ isLoading: detailLoading, opacity: 0.6 }"
              :class="['code-fullscreen', { 'full-active': isFullScreen }]"
            >
              <div class="col-aside-left">
                <div class="col-aside-left-header">
                  <bk-icon
                    @click="defectDetailDialogVisible = false"
                    class="arrows-left"
                    type="arrows-left"
                  />
              ...
              <div class="col-main">
                <b class="filename" :title="(currentTrace || {}).filePath">{{
                  ((currentTrace || {}).filePath || '').split('/').pop()
                }}</b>
                <div
                  ...
                  >
                    <div class="block">
                      <div class="item disb">
                        <span
                          class="fail"
                          :class="{ 'cc-status': currentFile.mark }"
                          v-if="currentFile.status === 1"
                        >
                        ...
                        <span class="success" v-else-if="currentFile.status & 2"
                        ><span class="cc-dot"></span>{{ $t('已修复') }}</span
                        >
                        <span class="warn" v-else-if="currentFile.status & 4"
                        ><span class="cc-dot"></span>{{ $t('已忽略') }}</span
                        >
                        <span
                          v-if="currentFile.status === 1 && currentFile.mark"
                          class="cc-mark disb"
                        >
                      ...
                      <div v-if="currentFile.status === 1" class="item">
                        <bk-button
                          ...
                          class="item-button"
                          @click="handleMark(0, false, currentFile.entityId)"
                        >
                          {{ $t('取消标记') }}
                        </bk-button>
                        <bk-button
                          ...
                          theme="primary"
                          class="item-button"
                          @click="handleMark(1, false, currentFile.entityId)"
                        >
                          {{ $t('标记处理') }}
                        </bk-button>
                      </div>
                      <div v-if="currentFile.status & 4">
                        <div class="item">
                          <bk-button
                            class="item-button"
                            @click="
                              handleRevertIgnoreAndMark(currentFile.entityId)
                            "
                          >
                            {{ $t('取消忽略并标记处理') }}
                          </bk-button>
                        ...
                        <div class="item">
                          <bk-button
                            class="item-button"
                            @click="
                              handleRevertIgnoreAndCommit(currentFile.entityId)
                            "
                          >
                            {{ $t('取消忽略并提单') }}
                          </bk-button>
                      ...
                      <div class="item">
                        <bk-button
                          v-if="currentFile.status & 4"
                          class="item-button"
                          @click="
                            handleIgnore(
                              'RevertIgnore',
                              false,
                              currentFile.entityId
                            )
                          "
                        >
                          {{ $t('取消忽略') }}
                        ...
                        <bk-button
                          v-else-if="!(currentFile.status & 2)"
                          class="item-button"
                          @click="
                            handleIgnore(
                              'IgnoreDefect',
                      ...
                      <div class="item" v-if="currentFile.status & 4">
                        <bk-button
                          class="item-button"
                          @click="handleChangeIgnoreType(currentFile)"
                        >
                          {{ $t('修改忽略类型') }}
                        </bk-button>
                      </div>
                      <div class="item">
                        <bk-button
                          ...
                          class="item-button"
                          @click="
                            handleCommit('commit', false, currentFile.entityId)
                          "
                        >
                          {{ $t('提单') }}
                        </bk-button>
                    ...
                    <div class="block">
                      <div class="item">
                        <dt>{{ $t('问题创建') }}</dt>
                        <dd>
                          {{ formatTime(currentFile.createTime, 'yyyy-MM-dd') }}
                        </dd>
                      </div>
                      <div class="item" v-if="currentFile.status & 2">
                        <dt>{{ $t('修复时间') }}</dt>
                        <dd>
                          {{ formatTime(currentFile.fixedTime, 'yyyy-MM-dd') }}
                        </dd>
                      </div>
                      <div class="item">
                        <dt
                    ...
                    <div class="block" v-if="currentFile.status & 4">
                      <div class="item">
                        <dt>{{ $t('忽略时间') }}</dt>
                        <dd>
                          {{ formatTime(currentFile.ignoreTime, 'yyyy-MM-dd') }}
                        </dd>
                      </div>
                      <div class="item">
                        <dt>{{ $t('忽略人') }}</dt>
      ...
      >
        ...
        <div slot="footer">
          <div
            ...
            @click="handleToIgnoreList"
          >
            <bk-button text>{{ $t('缺少合适忽略类型？') }}</bk-button>
          </div>
          <div
            v-else
            style="position: absolute; bottom: 18px; cursor: pointer"
            @click="handleToIgnoreList"
          >
            <bk-button text>{{ $t('新增忽略类型点此刷新') }}</bk-button>
            <i
              class="codecc-icon icon-refresh-2 f12 ml5"
              style="position: relative; bottom: 1px; color: #3a84ff"
            ></i>
          </div>
          <bk-button
            theme="primary"
      ...
      <bk-dialog
        v-model="changeHandlerVisible"
        theme="primary"
        :mask-close="false"
        :title="$t('跳转提示')"
      >
        <span>{{
          $t('配置处理人转换，可将原处理人当前和未来的问题都分配给新处理人。')
        }}</span>
        ...
        <template slot="footer">
          <bk-button theme="primary" @click="toLogs()">{{
            $t('去配置')
          }}</bk-button>
          <bk-button @click="changeHandlerVisible = false">{{
            $t('取消')
          }}</bk-button>
        </template>
      </bk-dialog>
      ...
      <div class="main-container large border-none">
        <div class="no-task">
          <empty
            ...
            :desc="
              $t(
                'CodeCC集成了十余款工具，支持检查代码缺陷、安全漏洞、代码规范等问题'
              )
            "
          >
            <template #action>
...
export default {
  ...
  computed: {
    ...
    typeTips() {
      return this.$t('起始时间x之后产生的问题为新问题', {
  ...
  watch: {
    ...
    defectDetailDialogVisible(val) {
      if (!val) {
        this.codeViewerInDialog.setValue('');
        this.codeViewerInDialog.setOption('firstLineNumber', 1);
      }
    },
    changeHandlerVisible: {
      handler() {
        if (!this.changeHandlerVisible) {
          window.localStorage.setItem(
            'memberNeverShow',
            JSON.stringify(this.memberNeverShow),
          );
        }
      },
      deep: true,
  ...
  methods: {
    ...
    getWarnContent(checkerKey) {
      if (
        this.checkerContentCache
        && this.checkerContentCache[this.checkerKey]
      ) {
        return this.checkerContentCache[this.checkerKey];
      }
      return this.$store.dispatch('defect/getWarnContent', {
        toolName: this.defectDetailSearchParams.toolName,
        checkerKey: this.checkerKey || checkerKey,
      });
    },
    keyOperate() {
      const vm = this;
      ...
      function keyDown(event) {
        ...
        switch (e.code) {
          ...
          case 'KeyW': // w
            if (e.shiftKey) {
              if (vm.defectDetailDialogVisible && vm.currentTraceIndex) {
          ...
          case 'KeyS': // s
            if (e.shiftKey) {
              if (
                vm.defectDetailDialogVisible
                && vm.currentTraceIndex < vm.traceDataList.length - 1
              ) {
                vm.currentTraceIndex += 1;
                const trace = vm.traceDataList[vm.currentTraceIndex];
                vm.clickTrace(trace);
    ...
    getDefectCountByType(type) {
      ...
      const count = this.listData[typeFieldMap[type]] || 0;
      return count > 100000 ? this.$t('10万+') : count;
    },
    ...
    buildLintHints(codeViewer, startLine, showDefectDetail) {
      ...
      defectList.forEach((defect) => {
        ...
        checkerDom.innerText = `${
          index && kind !== 'MULTI' ? `${index}.` : ''
        }${message || ''}`;
        ...
        hints.className = showDefectDetail
          ? `lint-hints active ${kind}`
          : `lint-hints ${kind}`;
    ...
    handleStatus(status, defectIssueInfoVO = {}) {
      ...
      let issueStatus = '';
      if (
        defectIssueInfoVO.submitStatus
        && defectIssueInfoVO.submitStatus !== 4
      ) {
    ...
    toChangeMember() {
      if (this.memberNeverShow) {
        ...
        this.changeHandlerVisible = true;
      }
    },
    ...
    getSearchParams() {
      ...
      const checkerSet = this.searchFormData.checkerSetList.find(checkerSet => checkerSet.checkerSetId === this.searchParams.checkerSet);
    ...
    generateExcel(list = []) {
      ...
      const title = `${this.taskDetail.nameCn}-${this.taskDetail.taskId}-${
        this.dimension
      }-${this.$t('问题')}-${new Date().toISOString()}`;
...
?entityId=${entityId}&status=${status}`;
    ...
    handleNextGuide() {
      if (!this.guideFlag) {
        document.getElementsByClassName('guide-icon')[0]
          && document.getElementsByClassName('guide-icon')[0].click();
        setTimeout(() => {
          document.getElementsByClassName('guide-flag')[0]
