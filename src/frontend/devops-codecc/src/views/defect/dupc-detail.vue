        ...
        <bk-col :span="1" class="source-code" id="sourceCodeCol">
          <div class="title-bar">
            ...
            <div class="select-chunk">
              <bk-dropdown-menu
                class="select-chunk-text"
                align="right"
                trigger="click"
              >
                <bk-button type="primary" slot="dropdown-trigger">
                  <span class="t1">{{
                    selectText.source.row || $t('点击选择重复源代码块')
                  }}</span>
                  <em class="t2" v-show="selectText.source.chunk">{{
                    $t('x块重复', { num: selectText.source.chunk })
                  }}</em>
                  <i
                    :class="[
...
export default {
  ...
  computed: {
    mergedLines() {
      ...
      const mergeResult = [
        [[sourceChunks[0].startLines, sourceChunks[0].endLines, 0]],
      ];
  ...
  methods: {
    ...
    updateEditor() {
      ...
      const [gutterLineNumber] = gutterElement.getElementsByClassName('CodeMirror-linenumbers');
    ...
    drawGutterLine() {
      ...
      const makeDotLine = function (lineId, lineIndex) {
        ...
        dotLine.addEventListener('click', (event) => {
      ...
      mergedLines.forEach((lines, i) => {
        ...
        lines.forEach((item, j) => {
          ...
          const line = [];
          for (
            let n = startLine - firstLineNumber;
            n <= endLine - firstLineNumber;
            n++
          ) {
    ...
    chunkAddClass(codeViewer, chunk) {
      ...
      codeViewer.eachLine((handle) => {
        codeViewer.removeLineClass(handle, 'wrap', 'background');
      });
      for (
        let i = startLines - trimBeginLine;
        i < endLines - trimBeginLine + 1;
        i++
      ) {
...
.chunk-list {
  ...
  box-shadow: 0 10px 15px 0 rgb(0 0 0 / 17%);
  user-select: none;
}
