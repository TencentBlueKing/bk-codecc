<template>
  <div class="cc-ignore-operation main-content-outer main-content-ignore-operation">
    ...
    <div class="operation-content" v-bkloading="{ isLoading }">
      <bk-form class="form-content" ref="form" :model="formData" :rules="isSysIgnore ? null : rules" :label-width="formLabelWidth">
        ...
        <div class="line"></div>
        <div class="review-setting-title">{{ $t('忽略问题 review 提醒设置') }}</div>
        <bk-form-item :label="$t('提醒日期')" ext-cls="date-select">
          <bk-select v-model="formData.notify.notifyMonths" multiple style="width: 150px;" @change="handleMonthsSelect">
            <bk-option
              v-for="(item, index) in monthsMap"
              :key="index"
              :id="item.value"
              :name="item.label"
              :class="{ 'is-selected': formData.notify.notifyMonths && (formData.notify.notifyMonths.includes(item.value) || formData.notify.notifyMonths.includes(-1)) }">
              <span>{{ item.label }}</span>
              <i v-if="formData.notify.notifyMonths && (formData.notify.notifyMonths.includes(item.value) || formData.notify.notifyMonths.includes(-1))"
                 class="bk-option-icon bk-icon icon-check-1"></i>
            </bk-option>
          </bk-select>
          <bk-select v-model="formData.notify.notifyWeekOfMonths" multiple style="width: 150px;" @change="handleWeekOfMonthsSelect">
            <bk-option
              v-for="(item, index) in weekOfMonthsMap"
              :id="item.value"
              :name="item.label"
              :key="index"
              :class="{ 'is-selected': formData.notify.notifyWeekOfMonths && (formData.notify.notifyWeekOfMonths.includes(item.value) || formData.notify.notifyWeekOfMonths.includes(-1)) }">
              <span>{{ item.label }}</span>
              <i v-if="formData.notify.notifyWeekOfMonths && (formData.notify.notifyWeekOfMonths.includes(item.value) || formData.notify.notifyWeekOfMonths.includes(-1))"
                 class="bk-option-icon bk-icon icon-check-1"></i>
            </bk-option>
          </bk-select>
          <bk-select v-model="formData.notify.notifyDayOfWeeks" multiple style="width: 150px;">
            <bk-option v-for="(item, index) in dayOfWeeksMap" :id="item.value" :name="item.label" :key="index"></bk-option>
          </bk-select>
        </bk-form-item>
        <bk-form-item :label="$t('通知人角色')">
          <bk-checkbox-group v-model="formData.notify.notifyReceiverTypes">
            <bk-checkbox value="ignore_author" class="mr40">{{ $t('问题忽略人') }}</bk-checkbox>
            <bk-checkbox value="defect_author" class="mr40">{{ $t('问题处理人') }}</bk-checkbox>
            <bk-checkbox value="task_creator">{{ $t('任务创建人') }}</bk-checkbox>
          </bk-checkbox-group>
        </bk-form-item>
        ...
        <bk-form-item :label="$t('通知方式')">
          <bk-checkbox-group v-model="formData.notify.notifyTypes">
            <bk-checkbox value="rtx" class="mr40">{{ $t('企业微信') }}</bk-checkbox>
...
<script>
  ...
  export default {
      ...
      return {
        ...
        formData: {
          ...
          notify: {
            ...
            notifyDayOfWeeks: [1],
            notifyReceiverTypes: ['ignore_author', 'defect_author', 'task_creator'],
            extReceiver: [],
    ...
    computed: {
      ...
      langType() {
        return window.navigator.systemLanguage ? window.navigator.systemLanguage : window.navigator.language
      },
      formLabelWidth() {
    ...
    methods: {
      ...
      handleConfirm() {
        this.$refs.form.validate().then(() => {
          ...
          const { notifyMonths, notifyWeekOfMonths, notifyDayOfWeeks,
                  notifyReceiverTypes, notifyTypes } = curData.notify
          let verify = false
          if ((!notifyMonths.length && !notifyWeekOfMonths.length && !notifyDayOfWeeks.length)
            || (notifyMonths.length && notifyWeekOfMonths.length && notifyDayOfWeeks.length)) {
            // 提醒日期三个都不选 或 三个都选择
            verify = true
          }  else  {
            verify = false
      ...
      handleMonthsSelect(newVal, oldVal) {
        ...
        if (type === 'add') {
          if (selectId === -1 || newVal.length === 12) {
            ...
            this.formData.notify.notifyMonths = this.monthsMap.filter(i => !newVal.includes(i.value))
              .map(_i => _i.value)
          }
        } else {
      ...
      handleWeekOfMonthsSelect(newVal, oldVal) {
        ...
        if (type === 'add') {
          if (selectId === -1 || newVal.length === 5) {
            ...
            this.formData.notify.notifyWeekOfMonths = this.weekOfMonthsMap.filter(i => !newVal.includes(i.value))
              .map(_i => _i.value)
          } else {
...
<style lang="postcss" scoped>
  .main-content-outer {
    ...
    background: #fff;
    border-radius: 2px;
    box-shadow: 0 2px 12px 0 hsla(0, 0%, 87%, .5), 0 2px 13px 0 hsla(0, 0%, 87%, .5);
    .operation-header {
      width: 100%;
      background: #fafbfd;
