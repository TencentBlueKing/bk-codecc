<template>
  <div class="cc-ignore main-content-outer main-content-ignore">
    <div class="ignore-content">
      <span v-bk-tooltips="{ content: $t('仅项目管理员或CI管理员可新增忽略类型'),
      ...
      <span class="title-tips">{{ $t('配置忽略类型，可定期提醒重新 review 相应忽略问题') }}</span>
      <bk-table
        ...
        <bk-table-column :label="$t('操作')" width="120">
          <template slot-scope="{ row }">
            <span class="mr10" v-bk-tooltips="{ content: $t('仅项目管理员或CI管理员可编辑忽略类型'),
                                                disabled: row.edit }">
              <bk-button text @click="handleEdit(row)" :disabled="!row.edit">{{ $t('编辑') }}</bk-button>
            </span>
            <span v-bk-tooltips="{ content: row.createFrom === 'sys' ? $t('系统内置忽略配置不可删除') : $t('仅项目管理员或CI管理员可删除忽略类型'),
                                   disabled: row.createFrom !== 'sys' && row.edit }">
              <bk-button text @click="handleDelete(row)" :disabled="countLoading || row.createFrom === 'sys' || !row.edit">{{ $t('删除') }}</bk-button>
            </span>
          </template>
        </bk-table-column>
        <div slot="empty">
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(row) {
        ...
        if (taskCount) {
          this.$bkMessage({ theme: 'error', message: '当前忽略类型问题或风险函数数量大于0，无法被删除。' })
          return
        }
        const params = {
          entityId,
        ...
        this.$bkInfo({
          ...
          confirmLoading: true,
          confirmFn: async () => {
            await this.$store.dispatch('ignore/deleteIgnore', params).then((res) => {
              const msg = res.data ? this.$t('忽略类型删除成功') : this.$t('忽略类型无法删除')
