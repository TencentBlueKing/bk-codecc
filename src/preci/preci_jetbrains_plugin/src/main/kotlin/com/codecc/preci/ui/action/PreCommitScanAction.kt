...
/**
 * Pre-Commit 增量扫描 Action
 *
 * 触发 PreCI pre-commit 增量扫描，只扫描 Git 暂存区中的变更文件。
 *
 * **功能说明：**
 * - 点击菜单项时触发
 * - 调用 ScanService.preCommitScan() 执行 pre-commit 扫描
 * - Local Server 会自动通过 Git 获取暂存区的变更文件
 * - 自动打开工具窗口并切换到进度选项卡
 * - 启动进度轮询，实时显示扫描状态
 * - 扫描结果通过 IDE 通知展示给用户
 *
 ...
 * @since 1.0
 */
class PreCommitScanAction : AnAction() {

    /**
     * 执行 pre-commit 增量扫描操作
     *
     * 在后台协程中调用 ScanService 执行 pre-commit 扫描，
     * 并自动打开工具窗口显示进度。
     *
